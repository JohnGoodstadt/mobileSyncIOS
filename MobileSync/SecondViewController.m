...
static NSString *const mobileDatabasePath = @"/mobile2.db";

@interface SecondViewController () <SendToServerWebserviceDelegate,GetRowsFromServerWebserviceDelegate,AccountHandlingWebserviceDelegate>
...
-(void)loginToServer
{
    ...
    NSString* domain = [MobileHelper getDomainID:self.db];
    ...
    [ws CallLogin:domain device_guid:device_guid login_id:USER_LOGIN_ID];
...
-(void)GetRowsFromServerWebserviceResponse:(NSDictionary*)package
{
    ...
    //[MobileHelper updateServerTimeStamp:self.db time_stamp:server_time];
...
-(void)AccountHandlingWebserviceResponse:(NSString*)server_time domain_id:(NSString*)domain_id{
    ...
    //A if domain id is the same as sent up then logged in for the first time
    ...
    //A.
    if([domain isEqualToString:domain_id])
    {
    ...
    else //B.
    {
        ...
        [self refreshFromServer]; //get all rows from new login
